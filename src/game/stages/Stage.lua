...
function Stage:rightCollision(entity, offset)
    ...
    if (( (entity.y+height)/self:getTileSize()) % math.floor( (entity.y+height)/self:getTileSize())) > 0.2 then
        row2 = row2 + 1

    end
    if col2 < self.colCount then -- bellow the right corner of the stage
        for i = math.max(1, row1), 
                math.min(row2,self.rowCount) do
            if self.map[i][col2-offset] ~= nil 
                    and self.map[i][col2-offset].solid then
                return true
            end -- end if local row2 = math.floor( (gameobject.y+height)/ts) --bottom
        end -- end for
    end -- end if    
    return false
end
...
function Stage:leftCollision(entity, offset)
    ...
    if col1 >= 1 then -- bellow the left corner of the stage
        for i = math.max(1, row1), 
                math.min(row2,self.rowCount) do
            if self.map[i][col1+1] ~= nil 
                    and self.map[i][col1+1].solid then
                return true
            end -- end if 
        end -- end for
    end -- end if    
...
function Stage:bottomHazard(entity, ofsRow, ofsCol)
    local row1,col1,row2,col2 = self:toMapCoords(entity)
    if row2+ofsRow < self.rowCount then -- not falling below screen
        for j = math.max(1, col1+ofsCol),
                math.min(col2-ofsCol, self.colCount) do
            if self.map[row2+ofsRow][j] ~= nil 
                    and self.map[row2+ofsRow][j].hazard then
                return true
            end -- end if solid tile
        end -- end for j
    end -- end if row2 < rowCount
...
function Stage:topCollision(entity, ofsRow, ofsCol)
    local row1,col1,row2,col2 = self:toMapCoords(entity)
    if row2+ofsRow < self.rowCount then -- not falling below screen
        for j = math.max(1, col1+ofsCol),
                math.min(col2-ofsCol, self.colCount) do
            if self.map[row1-ofsRow][j] ~= nil 
                    and self.map[row1-ofsRow][j].solid then
                return true
            end -- end if solid tile
        end -- end for j
    end -- end if row2 < rowCount
